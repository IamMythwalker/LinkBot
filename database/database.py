...
async def get_channels() -> List[int]:
    ...
    try:
        ...
        if not valid_channels:
            print(
                f"No valid channels found in database. Total documents checked: {
                    len(channels)}")
...
async def save_encoded_link(channel_id: int) -> Optional[str]:
    ...
    try:
        encoded_link = base64.urlsafe_b64encode(
            str(channel_id).encode()).decode()
        await channels_collection.update_one(
            {"channel_id": channel_id},
            {
                "$set": {
...
async def save_encoded_link2(
        channel_id: int, encoded_link: str) -> Optional[str]:
    """Save a secondary encoded link for a channel."""
    if not isinstance(channel_id, int) or not isinstance(encoded_link, str):
        print(
            f"Invalid input: channel_id={channel_id}, encoded_link={encoded_link}")
        return None
    ...
    try:
        ...
        print(
            f"Error saving secondary encoded link for channel {channel_id}: {e}")
        return None
...
async def get_channel_by_encoded_link2(encoded_link: str) -> Optional[int]:
    ...
    try:
        ...
        print(
            f"Error fetching channel by secondary encoded link {encoded_link}: {e}")
        return None


async def save_invite_link(
        channel_id: int, invite_link: str, is_request: bool) -> bool:
    ...
    if not isinstance(channel_id, int) or not isinstance(invite_link, str):
        print(
            f"Invalid input: channel_id={channel_id}, invite_link={invite_link}")
        return False
...
async def get_current_invite_link(channel_id: int) -> Optional[dict]:
    ...
    try:
        ...
        print(
            f"Error fetching current invite link for channel {channel_id}: {e}")
        return None
...
async def add_fsub_channel(channel_id: int) -> bool:
    ...
    try:
        ...
        await fsub_channels_collection.insert_one({
            ...
            'created_at': datetime.utcnow(),
            'status': 'active',
            'mode': 'join'  # Default mode when adding new channel
        })
        return True
    except Exception as e:
...
async def get_original_link(channel_id: int) -> Optional[str]:
    ...
    try:
        channel = await channels_collection.find_one({"channel_id": channel_id, "status": "active"})
        return channel.get(
            "original_link") if channel and "original_link" in channel else None
    except Exception as e:
...
# New FSub Mode Functions (Added to support the new feature)
async def set_fsub_mode(channel_id: int, mode: str) -> bool:
    """Set FSub mode for a channel (join/request)."""
    if mode not in ["join", "request"]:
        return False
    try:
...
async def get_fsub_mode(channel_id: int) -> str:
    """Get FSub mode for a channel (defaults to 'join')."""
    try:
        channel = await fsub_channels_collection.find_one({'channel_id': channel_id})
        return channel.get('mode', 'join') if channel else 'join'
    except Exception as e:
        print(f"Error getting FSub mode for channel {channel_id}: {e}")
        return 'join'


async def create_request_link(client, channel_id: int) -> str:
    """Create a request link with admin approval required"""
    try:
        ...
        print(f"Error creating request link for channel {channel_id}: {e}")
        return None
...
async def create_fsub_link(client: Bot, channel_id: int) -> str:
    """Generate the appropriate FSub link based on mode"""
