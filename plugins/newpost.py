...
async def set_channel(client: Bot, message: Message):
    ...
    try:
        ...
        if chat.permissions and not (
                chat.permissions.can_post_messages or chat.permissions.can_edit_messages):
        ...
        request_link = f"https://t.me/{
            client.username}?start=req_{base64_request}"
        reply_text = (
            f"<b><blockquote expandable>✅ Cʜᴀɴɴᴇʟ {
                chat.title} ({channel_id}) ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ.</b>\n\n"
...
async def send_channel_page(client, message, channels, page, edit=False):
    ...
    for channel_id in channels[start_idx:end_idx]:
        try:
            base64_invite = await save_encoded_link(channel_id)
            button_link = f"https://t.me/{
                client.username}?start={base64_invite}"
            chat = await client.get_chat(channel_id)
...
async def send_request_page(client, message, channels, page, edit=False):
    ...
    for channel_id in channels[start_idx:end_idx]:
        try:
            ...
            print(
                f"Error generating request link for channel {channel_id}: {e}")
...
async def send_links_page(client, message, channels, page, edit=False):
    ...
    for i, channel_id in enumerate(
            channels[start_idx:end_idx], start=start_idx + 1):
        try:
            chat = await client.get_chat(channel_id)
            base64_invite = await save_encoded_link(channel_id)
            normal_link = f"https://t.me/{
                client.username}?start={base64_invite}"
            base64_request = await encode(str(channel_id))
            await save_encoded_link2(channel_id, base64_request)
            request_link = f"https://t.me/{
...
async def send_channel_ids_page(
        client, message, channels, page, status_msg=None, edit=False):
    PAGE_SIZE = 10
    total_pages = (len(channels) + PAGE_SIZE - 1) // PAGE_SIZE
    start_idx = page * PAGE_SIZE
    end_idx = start_idx + PAGE_SIZE
